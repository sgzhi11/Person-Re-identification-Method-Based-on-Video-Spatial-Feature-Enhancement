...
class SRA(nn.Module):
    ...
    def forward(self, feat_map, re_featmap, Embeding_feature, feat_vect, aggregative_feature=None):
        ...
        Embeding_feat = Embeding_feature.view(b*t, -1,  h, w) # 128，128，16，8
        ...
        Gs_in = Gs.permute(0, 2, 1).view(b * t, h * w, h, w) # 128，128，16，8
        Gs_out = Gs.view(b * t, h * w, h, w)
        Gs_joint = torch.cat((Gs_in, Gs_out), 1) # 128，256，16，8

        Gs_joint = self.gg_spatial(Gs_joint) # 128，128，16，8
        para_spa = torch.cat((Embeding_feat, Gs_joint), 1) # 128，256，16，8
        ...
        for i in range(0, t, 2):
            ...
            feat_att = aggregative_feature[:, int(i/2), :, :, :] + self.relu(para_0 * feat_map[:, i, :, :, :] + para_1 * feat_map[:, i + 1, :, :, :])

            aggregative_feature_list.append(feat_att)
